 ...
 * validates boundaries, advances time by one week, and generates the result object.
 */
export function applyDecision(
  ...
  newGdp = isNaN(newGdp) ? prevGdp : Math.max(0, Number(newGdp.toFixed(2)));
  newInflation = isNaN(newInflation) ? prevInflation : Number(newInflation.toFixed(1));
  newUnemployment = isNaN(newUnemployment)
    ? prevUnemployment
    : Math.max(0, Number(newUnemployment.toFixed(1)));
  ...
  // 5. Generate Stat Changes array for Result Screen
  const statChanges: StatDiff[] = [];

  // Treasury
  const treasuryDelta = newTreasury - prevTreasury;
  statChanges.push({
    statKey: 'treasury',
    label: 'Treasury',
  ...
  statChanges.push({
    ...
    formattedNew: `₹ ${newGdp.toFixed(2)} T`,
    formattedDelta: `${gdpDelta >= 0 ? '+' : ''}${gdpDelta.toFixed(1)} T`,
    isPositiveGood: true,
  });

  // Inflation
  const inflationDelta = Number((newInflation - prevInflation).toFixed(1));
  statChanges.push({
    statKey: 'inflation',
    label: 'Inflation',
    ...
    formattedNew: `${newInflation.toFixed(1)}%`,
    formattedDelta: `${inflationDelta >= 0 ? '+' : ''}${inflationDelta.toFixed(1)}%`,
    isPositiveGood: false,
  });

  // Unemployment (if changed)
  const unemploymentDelta = Number((newUnemployment - prevUnemployment).toFixed(1));
  if (unemploymentDelta !== 0) {
    statChanges.push({
      ...
      formattedNew: `${newUnemployment.toFixed(1)}%`,
  ...
  const summary = `Executive Directive enacted for "${decision.title}". Action "${selectedOption.title}" implemented across key ministries. Time advanced to ${newTime.month}, Week ${newTime.week}, ${newTime.year}.`;
