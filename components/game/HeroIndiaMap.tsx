...
export const HeroIndiaMap: React.FC<HeroIndiaMapProps> = ({
  ...
  return (
    <div
      className={cn(
        'w-full h-[340px] sm:h-[380px] flex-1 min-h-[330px] rounded-game-lg glass-panel relative overflow-hidden border-2 border-gold/30 shadow-2xl flex flex-col justify-between p-3 select-none',
        className
      )}
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none p-2 opacity-45">
        <svg viewBox="0 0 400 450" className="w-full h-full text-gold/25 fill-current stroke-gold/50 stroke-1 object-contain">
      ...
      {/* West Node */}
      <div className="absolute top-1/2 left-10 -translate-y-1/2 flex items-center justify-center">
      ...
      <div className="absolute top-6 right-3 sm:right-6 z-20 flex flex-col items-end max-w-[210px]">
        {/* Pulsing Red Hazard Ring Marker */}
        <div className="relative flex items-center justify-center mb-1">
          <div className="absolute w-9 h-9 rounded-full bg-crimson/30 animate-ping" />
          <motion.div
        ...
        <motion.div
          ...
          onClick={onCrisisClick}
          className="glass-panel-crimson rounded-game p-2 flex flex-col gap-0.5 border-crimson/60 cursor-pointer shadow-2xl w-full"
        >
          <div className="flex items-center gap-1 text-crimson">
            <AlertTriangle className="w-3 h-3 shrink-0" />
            <span className="font-heading text-[10px] font-extrabold uppercase tracking-wide truncate">
              FLOOD IN ASSAM
            </span>
          </div>
          <p className="text-[9px] text-slate-200 font-sans leading-tight line-clamp-2">
            Heavy rainfall has caused severe flooding.
          </p>
          <div className="flex items-center justify-end gap-0.5 text-[9px] font-sans font-bold text-crimson hover:underline pt-0.5">
      ...
      <div className="absolute right-2.5 bottom-2.5 z-20 flex flex-col gap-1.5">
        <button
          ...
          className="w-10 h-10 rounded-game bg-navy-dark/90 border border-gold/30 flex flex-col items-center justify-center text-slate-300 hover:text-gold hover:border-gold shadow-lg transition-all"
        >
          <Radio className="w-3.5 h-3.5 text-gold" />
          <span className="text-[8px] font-sans font-bold tracking-tight uppercase mt-0.5">Intel</span>
        </button>
        ...
        <button
          ...
          className="w-10 h-10 rounded-game bg-navy-dark/90 border border-gold/30 flex flex-col items-center justify-center text-slate-300 hover:text-gold hover:border-gold shadow-lg transition-all"
        >
          <Handshake className="w-3.5 h-3.5 text-gold" />
          <span className="text-[8px] font-sans font-bold tracking-tight uppercase mt-0.5">Diplomacy</span>
        </button>
        ...
        >
          ...
          <span className="text-[8px] font-sans font-bold tracking-tight uppercase mt-0.5">World</span>
        </button>
      </div>
    </div>
  );
